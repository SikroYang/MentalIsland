...
<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>屏蔽关键词管理</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 搜索筛选 -->
    ...
    <el-table size="small" @selection-change="selectChange" :data="userData" highlight-current-row v-loading="loading"
      ...
      <el-table-column align="center" label="关键词" prop="Keyword" width="350">
        <!-- <template slot-scope="scope">
          <el-popover trigger="hover" placement="top">
            <p>{{ scope.row.Keyword }}</p>
    ...
    <el-dialog :title="title" :visible.sync="editFormVisible" width="60%" @click='closeDialog("edit")'>
      <el-form label-width="120px" ref="editForm" :model="editForm" :rules="rules">
        <el-form-item label="关键词" prop="Keyword">
          <el-input type="textarea" :autosize="{ minRows: 3, maxRows: 12 }" size="small" v-model="editForm.Keyword"
            auto-complete="off" placeholder='请输入关键词 (一个关键词一行 或两个关键词之间以"|"符号隔开)'></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods: {
    ...
    getdata(parameter) {
       ...
       */
      // 获取屏蔽关键词列表
      BlackWordList(parameter).then(res => {
        this.loading = false
        if (res.Type != "Success") {
          this.$message({
            type: 'info',
